window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const convEvents = ctx.get("uiConversation")?.events;
      ...
      // ── uiConversation 收集器：每轮 assistant 文本 → 提取路径存 turn data ──
      ...
      let bareHits = []; // [{ range, raw }]：当前高亮命中集合（点击命中测试用）
      let bareScanQueued = false;
      ...
      function collectBareHits() {
        // 跳过区（等同旧 skip 列表）：装饰已不做 DOM 变更，它不再是"崩溃防御"，
        // 而是"装饰边界"——消息区/输入区路径继续走 processPathCodes 与 turnTail。
        const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
          acceptNode(node) {
            const p = node.parentElement;
            if (p === null) return NodeFilter.FILTER_REJECT;
            if (p.closest("code, pre, a, button, script, style, [data-conversation-scroll], textarea, input, [contenteditable], [data-input-mirror], [data-input-backdrop], [data-composer-card], [class$='_input']")) return NodeFilter.FILTER_REJECT;
        ...
        while (walker.nextNode() !== null) {
          ...
          while ((m = BARE.exec(text)) !== null) {
            ...
            hits.push({ node, start: m.index, end: m.index + raw.length, raw });
            if (++count >= 8) break;
          }
        }
        return hits;
      ...
      function renderBareHighlights() {
        ...
        bareHits = hits.map((hit, i) => ({ range: ranges[i], raw: hit.raw }));
        const highlight = new Highlight(...ranges);
        CSS.highlights.set(BARE_HIGHLIGHT, highlight);
      }
      function scheduleBareScan() {
        if (bareScanQueued) return;
        bareScanQueued = true;
        requestAnimationFrame(() => {
          bareScanQueued = false;
      ...
      ctx.effect(() => {
        // 不再监听 characterData：击键不再触发全文档扫描（输入性能 + 竞争窗口一起消失）；
        // 只用 childList 覆盖结构变化（消息追加/切换会话等），rAF 合帧。
      ...
      }, "file-mentions: reveal buttons + bare path highlight");
