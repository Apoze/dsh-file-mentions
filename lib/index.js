  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const roots = probeRoots(settingsScope)
        const valid = []
  ...
  ctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      try {
        ...
        const cwd = session && session.header && typeof session.header.cwd === 'string' ? session.header.cwd : null
        const extras = await durableCwds(workspaceRegistry, sessionPersistence)
        const abs = await resolveFirst(path, cwd, sessions, extras, probeRoots(settingsScope))
        if (abs === null) {
    ...
    handler: async (req, res) => {
      try {
        ...
        if (req.method === 'GET' || req.method === 'HEAD') {
          writeJson(res, 200, { ok: true, extraProbeRoots: probeRoots(settingsScope) })
          return
        }
        if (req.method === 'POST' || req.method === 'PUT') {
          ...
          await settingsScope.update({ extraProbeRoots: next })
 ...
 * The caller passes its registered settings scope; disposal removes that scope.
 */
function probeRoots(settingsScope) {
  try {
